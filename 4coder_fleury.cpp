...
#include "4coder_fleury_nickel.cpp"
...
void custom_layer_init(Application_Links *app)
{
	...
	// NOTE(rjf): Set up hooks.
	{
		...
		set_custom_hook(app, HookID_NewFile,                 nickel_new_file_hook);
...
CUSTOM_DOC("Fleury startup event")
{
	...
	//~ NOTE(rjf): Initialize panels
	{
		...
		{
			compilation_view = open_view(app, right_view, ViewSplit_Bottom);
			...
			// NOTE(nickel): made compilation height higher from 4.0 to 10.0
			i32 compilation_height = (i32)(metrics.line_height * 10.f);
			view_set_split_pixel_size(app, compilation_view, compilation_height);
