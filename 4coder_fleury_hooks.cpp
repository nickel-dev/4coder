                ...
                Rect_f32 rect, Frame_Info frame_info)
{
	...
	//~ NOTE(nickel): Hex color highlight
	{
		Scratch_Block scratch(app);
...
F4_Render(Application_Links *app, Frame_Info frame_info, View_ID view_id)
{
	...
	//~ NOTE(rjf): Draw background.
	{
		...
		{
			// NOTE(nickel): Sets compilation color to be black
...
function BUFFER_HOOK_SIG(F4_BeginBuffer)
{
	...
	*eol_setting = setting;
	...
	// NOTE(nickel): I would like to see my error messages in full
	/*if(string_match(buffer_name, string_u8_litexpr("*compilation*")))
	{
			wrap_lines = false;
	}*/
